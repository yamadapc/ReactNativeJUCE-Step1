...
SCENARIO ("object[\"property\"]")
{
    WHEN ("getting a property")
    {
        ...
        THEN ("getProperty is equivalent to operator[]")
        {
            auto eitherValue = context.evaluateScript ("const something = {here: 10}; something");
            REQUIRE (!eitherValue);
            auto value = eitherValue.left ().unsafeGet ();
            REQUIRE (value.isObject ());
            auto obj = value.get<CJSObject> ();
            REQUIRE (obj.getProperty ("here").isStrictEqual (obj["here"]));
            REQUIRE (obj["here"].isStrictEqual (obj["here"]));
            REQUIRE (obj["here"] == obj["here"]);
        }
    }
}

SCENARIO ("array[index]")
{
    WHEN ("getting an index")
    {
        CJSContext context;

        THEN ("getProperty is equivalent to operator[]")
        {
            auto eitherValue = context.evaluateScript ("const something = [0, 1, 2, 3]; something");
            REQUIRE (!eitherValue);
            auto value = eitherValue.left ().unsafeGet ();
            REQUIRE (value.isObject ());
            auto arr = value.get<CJSObject> ();
            REQUIRE (arr.getPropertyAtIndex (2).isStrictEqual (arr[2]));
            REQUIRE (arr[2].isStrictEqual (arr[2]));
            REQUIRE (arr[2] == arr[2]);
        }
    }
}
